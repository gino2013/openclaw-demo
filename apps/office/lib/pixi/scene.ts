...
function updateConnections(): void {
  ...
  const hub = sprites[0]!
  const hubCx = hub.container.x + 12
  const hubCy = hub.container.y + 12

  for (let i = 1; i < sprites.length; i++) {
    const s = sprites[i]!
    const cx = s.container.x + 12
    const cy = s.container.y + 12
...
export function triggerDelegationAnimation(
  ...
  const fp = { x: from.container.x + 12, y: from.container.y + 12 }
  const tp = { x: to.container.x + 12,   y: to.container.y + 12 }
